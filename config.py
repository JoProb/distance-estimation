...
class Config:
    ...
    min_depth: float = 1.0  # minimum sampled distance
    max_depth: float = 25.0  # maximum sampled distance
    ...
    detect_humans: bool = (
        False  # whether to detect humans and include their distance estimations in the output
    )

    # depth estimation parameters
    depth_estimation_model: DepthEstimationModel = (
        DepthEstimationModel.DPT_PYTORCH
    )  # one of DPT|DEPTH_ANYTHING
    ...
    draw_world_position: bool = (
        False  # whether to annotate estimated world position over detected bounding boxes
    )

    # calibration parameters
    calibrate_metric: bool = False  # whether to calibrate in metric or in disparity space
    calibration_regression_method: RegressionMethod = (
        RegressionMethod.RANSAC
    )  # one of RANSAC|LEASTSQUARES|POLY|RANSAC_POLY
    ...
    # sampling_parameters
    detection_sampling_method: DetectionSamplingMethod = (
        DetectionSamplingMethod.BBOX_PERCENTILE
    )  # one of BBOX_BOTTOM|BBOX_PERCENTILE|SAM
    multiple_animal_reduction: MultipleAnimalReduction = (
        MultipleAnimalReduction.NONE
    )  # one of NONE|MEDIAN|ONLY_CENTERMOST|
